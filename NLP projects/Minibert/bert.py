...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    key1 = key.transpose(-1,-2)
    S = torch.matmul(query, key1)
    ...
    attention_probabilities = F.softmax(S, dim=-1)
    # Step 5: Apply dropout to the attention to get the final attention scores
    attention_scores = self.dropout(attention_probabilities)
    ...
    V = V.view(bs, -1, self.num_attention_heads * self.attention_head_size)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    temp_output = self.interm_dense(attention_output)

    # Step 4: Apply activation function to the output of feed forward layer using self.interm_af
    temp_output = self.interm_af(temp_output)

    # Step 5: Apply another add-norm layer using self.add_norm
    layer_output = self.add_norm(attention_output, temp_output, self.out_dense, self.out_dropout, self.out_layer_norm)
